...
export default function InstallationsPage() {
  ...
  const nextStep = () => {
    ...
    if (currentStep === 3) {
      // Auto-fill amount paid to total if moving to step 4 initially
              ...
              {currentStep === 3 && (
                <div className="flex flex-col gap-6 animate-in slide-in-from-right-4 duration-300">
                  <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
                    ...
                    <h3 className="text-sm font-extrabold text-slate-900 uppercase tracking-widest">Phase 3: Confirm Items & Prices</h3>
                  </div>

                  <div className="bg-slate-50/50 border border-slate-200 rounded-2xl overflow-hidden text-xs shadow-sm">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-100 border-b border-slate-200 text-[10px] font-bold uppercase tracking-wider text-slate-500">
                          <th className="py-3 px-5 font-bold">Item Description</th>
                          <th className="py-3 px-5 font-bold text-right w-44">Price (₹)</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-200">
                        {form.modelName && (
                          <tr className="bg-white">
                            <td className="py-4 px-5 font-bold text-slate-800 uppercase">
                              {form.modelName} <span className="text-[10px] text-slate-400 block font-normal normal-case mt-1">RO Machine</span>
                            </td>
                            <td className="py-3 px-5">
                              <div className="relative">
                                ...
                                <input 
                                  ...
                                  className="w-full pl-7 pr-3 py-2.5 font-bold text-right border border-slate-200 rounded-xl outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 bg-white text-sm"
                                />
                              </div>
                            </td>
                          </tr>
                        ...
                        {form.selectedEquipments.map((eq, idx) => (
                          <tr key={eq.name} className="bg-white">
                            <td className="py-4 px-5 font-bold text-slate-800">
                              {eq.name} <span className="text-[10px] text-slate-400 block font-normal mt-1">Extra Equipment / Part</span>
                            </td>
                            <td className="py-3 px-5">
                              <div className="relative">
                                <span className="absolute left-3 top-1/2 -translate-y-1/2 font-bold text-slate-500">₹</span>
                        ...
                        {form.amcPlanName && (
                          <tr className="bg-white">
                            <td className="py-4 px-5 font-bold text-slate-800">
                              {form.amcPlanName} <span className="text-[10px] text-slate-400 block font-normal mt-1">AMC Service Plan</span>
                            </td>
                            <td className="py-3 px-5">
                              <div className="relative">
                                <span className="absolute left-3 top-1/2 -translate-y-1/2 font-bold text-slate-500">₹</span>
                        ...
                        {!form.modelName && form.selectedEquipments.length === 0 && !form.amcPlanName && (
                          <tr className="bg-white">
                            <td colSpan={2} className="py-8 px-5 text-center text-slate-400 font-medium">
                              No items selected. Please go back to select hardware or service plans.
                            </td>
                          </tr>
                        )}
                      </tbody>
